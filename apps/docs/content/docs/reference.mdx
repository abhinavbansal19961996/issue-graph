...
The command runs `twg jira workitem get ISSUE-KEY --full` with direct JSON output. It reads Jira only, uses TWG credentials without copying tokens, and writes no snapshots. Same-project links recurse; cross-project issues are fetched at the boundary; non-Jira links are reported but not fetched. JSON has `schemaVersion: 1` and explicit coverage. Exit 1 preserves partial data when reads fail or the node cap is reached.
